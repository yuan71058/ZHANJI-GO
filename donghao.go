...
type Result struct {
	Code   int         `json:"code"`   // 返回状态码：200或1=成功，其他=失败
...
const (
	...
	// ENC_CHACHA  = 6 // ChaCha20-Poly1305 - 已禁用: OpenSSL 1.1.1 不支持AEAD Tag
)
...
//
// 如果存储中已有客户端ID，则返回已有ID；如果没有，则生成新的ID并返回
//
// 参数:
...
// OpenSSL 1.1.1 不支持AEAD Tag，导致加密解密不兼容
